...
- 粤语专用 CosyVoice2：`D:\shenggui\CosyVoice2-Yue\pretrained_models\yue\CosyVoice2-Yue-ZoengJyutGaai\CosyVoice2-yue-zjg`
- 吴语专用 CosyVoice2：`D:\shenggui\CosyVoice2-Wu\pretrained_models\ASLP-lab\WenetSpeech-Wu-Speech-Generation\CosyVoice2`
- 西南官话专用 CosyVoice2：`D:\shenggui\CosyVoice2-Chuan\pretrained_models\chuan\CosyVoice2-Chuan`
...
- `POST /api/clone-preview`：上传用户参考音频，按当前方言调用对应 CosyVoice2 微调模型，返回镜像示范结果。

## 模型接入位置

- `app/settings.py`：环境变量和本地绝对路径。
- `app/services/model_adapters.py`：SenseVoice llama-funasr / FunASR 与 CosyVoice2 方言微调适配层。
...
- [2026-07-24 CosyVoice2 方言模型路由更新](docs/update-2026-07-24-cosyvoice2-dialect-routing.md)
